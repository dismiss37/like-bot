...
# --- DEFAULT VALUES ---
BOT_TOKEN = "7965256274:AAFpQ9JEF-d5qefe8pPNR5zr0Kjewt247yo"
...
if __name__ == "__main__":
    ...
    # Yahan badlav kiya gaya hai taaki bot error aane par band na ho
    while True:
        try:
            bot.infinity_polling(timeout=60, long_polling_timeout=60)
        except telebot.apihelper.ApiTelegramException as e:
            if e.result.status_code == 409:
                print("Conflict error (409) detected. Restarting polling in 5 seconds...")
                time.sleep(5)
            else:
                print(f"An unexpected API error occurred: {e}")
                time.sleep(10) # Dusre errors ke liye thoda zyada wait karein
        except Exception as e:
            print(f"An error occurred: {e}")
            time.sleep(10) # Baaki sabhi errors ke liye wait karein
