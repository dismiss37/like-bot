...
def handle_like(message):
    ...
    if remaining <= 0:
        bot.reply_to(message, "🚫 Limit reached! Please try again later.")
        return
        ...
        if response.status_code == 200:
            ...
            if status == 1:
                ...
                text = (
                    ...
                    f"🎚️ <b>Player Level: {response_data.get('PlayerLevel', 'N/A')}</b>\n"
...
def handle_remain(message):
    if message.chat.id not in ALLOWED_GROUPS:
        ...
        return
    remaining = get_remaining_likes()
    bot.reply_to(message, f"📊 **Remaining Requests:** {remaining}/{GLOBAL_LIMIT}")
...
def handle_admin_commands(message):
    if message.from_user.id != ADMIN_USER_ID:
        bot.reply_to(message, "🚫 Access Denied! This command is for the admin only.")
        return
